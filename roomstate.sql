...
(
    ...
    `Booking_roomstate`	                                               BOOL             NULL        COMMENT '객실 이용현황',    
...
(select room.room_id,
      Case 
         when (current_date() between booking.booking_checkindate and booking.booking_checkoutdate) then '1'
            else '0'
      end as case_roomstate,
      booking.cust_id, booking.booking_id from room cross join booking);

update booking
   set Booking_roomstate = 
      (Case 
         when Current_date() between booking_checkindate and booking_checkoutdate then '1'
         else '0' end )
	where booking_id>0;
 
UPDATE roomstate, booking
SET roomstate.roomstate_state = booking.booking_roomstate
WHERE roomstate.booking_ID = booking.booking_ID;
